            ...
            <template>
                <div class="flex flex-wrap mb-4">
                    <div class="w-1/3 px-3 mobile:w-full mobile:mb-3">
                        <label class="block mb-3" for="inventory_type">
                    ...
                    <div class="w-1/3 px-3 mobile:w-full mobile:mb-3" v-if="['character', 'motel', 'property'].includes(filters.inventory_type)">
                        <label class="block mb-4 font-semibold" for="inventory_cid">
                            {{ t('inventories.search.cid') }}
                        </label>
                        <input class="w-full px-4 py-2 bg-gray-200 dark:bg-gray-600 border rounded" id="inventory_cid" v-model="filters.inventory_cid" placeholder="16802">
                    </div>

                    <div class="w-1/3 px-3 mobile:w-full mobile:mb-3" v-if="filters.inventory_type === 'vehicle'">
                        <label class="block mb-4 font-semibold" for="inventory_plate_id">
                            {{ t('inventories.search.plate_id') }}
                        </label>
                        <input class="w-full px-4 py-2 bg-gray-200 dark:bg-gray-600 border rounded" id="inventory_plate_id" v-model="filters.inventory_plate_id" placeholder="24YNM199 / 1234">
                    </div>

                    <div class="w-1/3 px-3 mobile:w-full mobile:mb-3" v-if="filters.inventory_type === 'evidence'">
                        <label class="block mb-4 font-semibold" for="inventory_evidence_id">
                            {{ t('inventories.search.evidence_id') }}
                        </label>
                        <input class="w-full px-4 py-2 bg-gray-200 dark:bg-gray-600 border rounded" id="inventory_evidence_id" v-model="filters.inventory_evidence_id" placeholder="24YNM199 / 1234">
                    </div>

                    <div class="w-1/3 px-3 mobile:w-full mobile:mb-3">
                        <label class="block mb-4">&nbsp;</label>
                        <button class="block w-full px-4 py-2 font-semibold text-center text-white bg-success rounded dark:bg-dark-success" @click="refresh">
                            <span v-if="!isLoading">
